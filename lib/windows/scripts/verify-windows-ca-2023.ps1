...
try {
    ...
    Write-State "Secure Boot enabled" "FAILED: $($_.Exception.Message)"
}
...
try {
    ...
    Write-State "DBX read" "FAILED: $($_.Exception.Message)"
}
...
foreach ($f in $bootFiles) {
    ...
    $tag = if (-not $issuer)                              { 'signature unreadable' }
           elseif ($issuer -match 'Windows UEFI CA 2023') { 'CA 2023' }
...
try {
    ...
    if (Test-Path $espPath) {
        ...
        $tag = if (-not $espIssuer)                              { 'signature unreadable' }
               elseif ($espIssuer -match 'Windows UEFI CA 2023') { 'CA 2023 (firmware MIGRATED)' }
    ...
    Write-State "ESP read" "FAILED: $($_.Exception.Message)"
} finally {
...
$statusStr  = if ($null -ne $status)    { $status.ToString() }     else { "(not set)" }
